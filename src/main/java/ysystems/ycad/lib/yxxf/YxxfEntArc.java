...
//==============================================================================
// $Header: /cvsroot/ycad/ycad/src/com/ysystems/ycad/lib/yxxf/YxxfEntArc.java,v 1.23 2003/05/08 11:24:03 ekarlo Exp $
// $Log: YxxfEntArc.java,v $
...
//
// Revision 1.21  2000/10/17 07:43:59  ekarlo
// Change package paths to lower case.
//
// Revision 1.20  1999-09-15 10:08:11-06  walter
// Added JavaDoc comments.
//
// Revision 1.19  1999-08-10 08:06:35-06  ekarlo
...
// Rearrange package names.
//
// Revision 1.15  1999/02/08  05:10:53  ekarlo
...
//
// Revision 1.2  1996/07/30  06:49:38  ekarlo
// Use point class.
//
 ...
 */
public class YxxfEntArc extends YxxfEntHeader implements YxxfEnt
{
    /**
     * code  10,20,30
     *            Center (in OCS).
    ...
    double                      radius          = 0.0;
    /**
     * code  50 - Start angle (in degrees).
     */
    public
    double                      entbegang       = 0.0;
    /**
     * code  51 - End angle (in degrees).
     */
    public
    double                      entendang       = 0.0;
    ...
    {
        if (gc.setupEntity(this, M_arc) == false)
            return;

        // Determine type of arc (line type, orientation)
        // (Arcs do not have width)
        ...
        {   // Continuous line
            ...
            {   // Face off view
                ...
                {   // Face off view - with thickness - 3D wire frame only
                    gc.drawCircle__ECS__cont__thck__wid_none(center, radius, begang, swgang, thickness);
                }
            }
        }
        else
        {  // Pattern line
            if (gc.check_xtrudir_parallel_to_view())
            {   // Direct face on view - thickness ignored
                gc.drawCircle__ECS__ltyp__flat__wid_none(center, radius, begang, swgang);
            }
            else
            {   // Face off view
                if (thickness == 0.0)
                ...
                {   // Face off view - with thickness - 3D wire frame only
                    gc.drawCircle__ECS__ltyp__thck__wid_none(center, radius, begang, swgang, thickness);
                }
            }
        }
    }
    ...
    void calc(Yxxf D)
    {
        ...
        // Initialize arc matrix
        if (M_arc == null)
            M_arc = new YxxfGfxMatrix();
        else
            M_arc.mtxSetIdentity();
        ...
        //System.out.println("ARC:after calcAAA");
        //System.out.println("ARC:    Ax =" + Ax + ",dist=" + Ax.distance());
        //System.out.println("ARC:    Ay =" + Ay + ",dist=" + Ay.distance());
        //System.out.println("ARC:    Az =" + Az + ",dist=" + Az.distance());


        // Apply rotation
        M_arc.mtxRotateAxes_World_to_Local(Ax, Ay, Az);


//      // Calculate Cp from center
//      // Applies the current center rotation
//      Cp.set(center);
//      M_arc.mtxTransformPoint(Cp);
//      //System.out.println("ARC:    center =" + center + ",dist=" + center.distance());
//      //System.out.println("ARC:    Cp     =" + Cp + ",dist=" + Cp.distance());
//
//
//      // Apply translate (center point)
//      M_arc.mtxTranslate(Cp);
//
//
        // Preprocess angles
        begang = entbegang * (Math.PI / 180.0);
        swgang = ((entbegang > entendang) ? (entendang - entbegang + 360.0) :
                                            (entendang - entbegang)) * (Math.PI / 180.0);
    }
}
